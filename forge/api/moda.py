...
class InitResponse(_CamelModel):
  ...
  state: SimState
  config: Config
  # Captured stdout from the snippet(s) executed for this call.
  # `print()` output inside MoDa snippets surfaces here so the React
  # iframe can render it in its console panel. Empty string when the
  # snippet didn't print. Mirrors server.py's generic /compute
  # `stdout` field — same name, same shape.
  stdout: str = ""
...
def _run_snippet(snippet_id: str, args=(), inputs=None):
  ...
  Mirrors the action-snippet branch of server.py's /compute. Returns
  a `(stdout, result)` tuple — the raw captured stdout (from any
  `print()` calls inside the snippet) and the raw Python result
  (whatever the snippet's compute(...) returned). Data snippets
  return `("", value)` so the tuple shape is uniform.
  """
  inputs = inputs or {}
  ...
  if snippet_type in ("data", "snapshot"):
    # Data snippets don't execute Python — no stdout to capture.
  ...
  try:
    ...
    # Preserve any stdout captured before the snippet failed, mirroring
    # server.py's /compute behavior. The React iframe surfaces this in
    # its console panel even when the run errored.
    raise HTTPException(
      status_code=500,
      detail={
        "error": f"snippet {snippet_id!r} execution failed: {e}",
        "stdout": e.stdout,
      },
    )
...
def init(req: InitRequest) -> dict:
  ...
  stdout, particle_state = _run_snippet("setup", args=("medium",))
...
def compute(req: ComputeRequest) -> dict:
  ...
  stdout, new_state = _run_snippet("go", args=(particle_state, req.dt, req.temperature))
...
def click(req: ClickRequest) -> dict:
  ...
  start = time.perf_counter()
  stdout, new_state = _run_snippet("on_mouse_click", args=(particle_state, req.x, req.y))
  elapsed_ms = (time.perf_counter() - start) * 1000
  logger.info("moda /click: elapsed_ms=%.1f", elapsed_ms)

  SESSIONS[req.session_id] = new_state
  return ClickResponse(stdout=stdout).model_dump(by_alias=True)
